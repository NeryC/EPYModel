...
const fakePoint = (fecha: string, value: number): DataPoint => ({ fecha, value });
...
describe('setChecks reducer', () => {
  it('toggles a boolean setting on the given main graph type', () => {
    ...
    const s1 = graphInfoReducer(s0, setChecks({ type: 'reported', checkName: 'isSmooth' }));
...
describe('setRange reducer', () => {
  it('updates the range of a given main graph type', () => {
    ...
    expect(state.main.reported.settings.range).toEqual({ start: 0, finish: 100 });
  });
});
...
describe('selectors', () => {
  it('selectGraphData returns isReady false when state is empty', () => {
    ...
    const result = selectGraphData({ main: undefined, simulation: undefined } as any);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
