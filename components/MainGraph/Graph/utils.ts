...
export const drawLines = (
  ...
  svgChart: d3.Selection<SVGElement, unknown, null, undefined>,
  uncertainty: boolean,
  type: string,
  xScale: d3.ScaleTime<number, number>,
  yScale: d3.ScaleLinear<number, number>,
  data: DataPoint[],
  xAxisGroup: d3.Selection<SVGElement, unknown, null, undefined>,
  yAxisGroup: d3.Selection<SVGElement, unknown, null, undefined>
): void => {
...
export const getMaxField = (data: DataPoint[], lines: (LineItem | string)[]): string => {
  ...
  for (let x = 0; x < lines.length; x++) {
    const fieldName = typeof lines[x] === "string" ? lines[x] : (lines[x] as LineItem).name;
 ...
 */
export function timeFormat(formats: Array<[string, (date: Date) => boolean]>): (date: Date) => string {
...
export const createZoom = (
  ...
  width: number,
  height: number,
  zoomed: (event: d3.D3ZoomEvent<SVGElement, unknown>) => void
