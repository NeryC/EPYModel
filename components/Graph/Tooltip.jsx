...
export default function Tooltip ({data,tooltipRef,activeLines}) {
  ...
  const chooseClass = (fieldName) => (fieldName == "Reportados" ? 'circle mx-auto' : 'border_line w-3.5');
      ...
      <table className="table-fixed">
        <tbody className="text-gray-700">
          {showedLines.map((line, index) => {
            ...
            <tr
              key={index}
              className={shouldShow(line.name)}
            >
              <td className='pr-1.5'>
                <div className={chooseClass(line.name)} style={chooseLineColor(line.name)}/>
              </td>
              <td className='pr-1.5'>
                {`${line.label}: `}
              </td>
              <td className="font-bold">
                {Math.round(data[line.name])}
              </td>
            </tr>
          )})}
        </tbody>
